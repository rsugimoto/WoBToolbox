...
analytical_solution(ANALYTICAL_SOLUTION_TYPE solution_type, const Eigen::Matrix<ScalarType, Dim, 1> &y) {
    ...
    case EXTERIOR_SOLUTION1:
        return Eigen::Matrix<ScalarType, Dim + 1, 1>(
            y[0] / y.squaredNorm(), 1.f / y.squaredNorm() - 2.f * y[0] * y[0] / (y.squaredNorm() * y.squaredNorm()),
            -2.f * y[0] * y[1] / (y.squaredNorm() * y.squaredNorm())
        );

    case INTERIOR_SOLUTION1: return Eigen::Matrix<ScalarType, Dim + 1, 1>(y[1], 0.0, 1.0);

    case INTERIOR_SOLUTION2:
        const ScalarType scale = (ScalarType)M_PI;
        return Eigen::Matrix<ScalarType, Dim + 1, 1>(
            std::cos(scale * y[0]) * std::sinh(scale * y[1]), -scale * std::sin(scale * y[0]) * std::sinh(scale * y[1]),
            scale * std::cos(scale * y[0]) * std::cosh(scale * y[1])
        );
    }
    return Eigen::Matrix<ScalarType, Dim + 1, 1>();
}
...
ScalarType get_neumann_bc(
    ...
    Eigen::Matrix<ScalarType, Dim, 1> gradient = analytical_solution(solution_type, y).tail<2>();
...
std::tuple<ScalarType, ScalarType, ScalarType> compute_rmse(
    ...
    return {solution_rmse, x_gradient_rmse, y_gradient_rmse};
}
...
int main(int argc, char *argv[]) {
    ...
    for (size_t f = 0; f < F.rows(); f++) {
        ...
        if (boundary_type == wob::Dirichlet)
            elems.push_back(
                {v1, v2, n, get_dirichlet_bc(solution_type, v1), get_dirichlet_bc(solution_type, v2), wob::Dirichlet}
            );
        else if (boundary_type == wob::Neumann)
            elems.push_back(
                {v1, v2, n, get_neumann_bc(solution_type, v1, n), get_neumann_bc(solution_type, v2, n), wob::Neumann}
            );
        else if (boundary_type == wob::Robin)
            elems.push_back(
                {v1, v2, n, get_dirichlet_bc(solution_type, v1) + get_neumann_bc(solution_type, v1, n),
                 get_dirichlet_bc(solution_type, v2) + get_neumann_bc(solution_type, v2, n), wob::Robin, 1.0f, 1.0f}
            );
    }
    ...
    for (size_t i = 0; i < num_evaluation_points; i++) {
        ...
        if (domain == wob::InteriorDomain ? winding_number < (ScalarType)-0.5 : winding_number >= (ScalarType)-0.5) {
            ...
            anaylical_gradient_buffer[i] = analytical_solution_gradient.tail<2>();
    ...
    wob::utils::save_field(anaylical_gradient_buffer, output_dir + "/gradient_analytical.vector2", grid_res);
    ...
    while (elapsed_time < max_time * 60.0) {
        ...
        auto [solution_rmse, x_gradient_rmse, y_gradient_rmse] =
            compute_rmse(_solution, _gradient, evaluate_point_host, grid_res, solution_type, problem);

        std::cout << "\33[2K\r"; // clear line
        std::cout << "time " << elapsed_time << " s\t" << num_iterations << " iterations (" << total_sample_count
                  << " samples)\t" << std::flush;
        std::cout << "RMSE : " << solution_rmse << " " << x_gradient_rmse << " " << y_gradient_rmse << std::flush;
        rmse_out << total_sample_count << "," << elapsed_time << "," << solution_rmse << "," << x_gradient_rmse << ","
                 << y_gradient_rmse << std::endl;
